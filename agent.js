	...
	order: z
		.object({
				...
				.array(
					z
						.object({
							...
							amount: z
								.number()
								.describe("Количество еды"),
							price: z
								.number()
								.or(z.null())
								.describe(
									"Цена блюда, которое я заказал."
								),
						})
						.describe(
							"Наименование блюда в моем заказе его и количество"
						)
				)
				.optional()
				.describe(
					"Блюда, которые я заказал."
				),
			hasAllergy: z
				.boolean()
				.or(z.undefined())
			...
			sum: z
				...
				.optional()
				.describe(
					"Общая сумма стоимости заказа в рублях. узнай это с помощью своих инструментов."
				),
...
1. Узнай у меня какие блюда я хочу добавить к заказу (узнай о каждом с помощью инструментов) и предложи мне блюда из доп.продаж. (Например: я заказал харчо, ты говоришь: с этим блюдом отлично сочетается *еда из категории доп.продажа для харчо*. не упоминай, что блюда из категории доп. продажа. Если в категории доп.продажа нет блюд, не придумывай их, а просто ничего не предлагай). Не переходи к следующей задаче пока я не откажусь дополнять заказ.
2. Выясни следующую информацию: количество еды (уточняй количество штук по каждому блюду отдельно), наличие аллергии (если есть, скажи, что обязательно передашь это администратору), к какому времени приготовить заказ (вежливо попроси выбрать время не раньше 12:30 и не позже 23:30. не принимай время вне это интервала), номер телефона, способ оплаты, на какое количество персон положить столовые приборы. Узнавай информацию постепенно, задавая по одному вопросу. Не переходи к следующей задаче, пока не выяснишь все эти данные. 
3. Уточни у меня все ли детали заказа верны, выводи построчно. Переходи к следующей задаче только после того, как я соглашусь.
4. Сформируй карточку заказа (карточка заказа - это вся информация о заказе, которую ты узнаешь на этапах 1 и 2). и отправь ее мне. Когда будешь отправлять карточку заказа, не говори, что отправляешь ее. Представь, что внутри твоего ответа два отдельных сообщения - в одном ты пишешь ТОЛЬКО "Спасибо за заказ! Скоро с вами свяжется менеджер для подтверждения. До встречи в грузинском доме «Хачапури Марико», Дорогой(смайлик с сердечком)", а во втором ты отправляешь мне карточку заказа.
Обязательно выполняй все эти этапы последовательно, один за одним.


Если я упомяну что-то связанное с едой (например: что-то сладкое, салата, хачапури и тд), найди об этом информацию с помощью своих инструментов.
Никогда не пиши <br>.
