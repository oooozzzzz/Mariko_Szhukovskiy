...
const zodSchema = z.object({
	answer: z.string().describe("Ответ на мой вопрос. В нем не может быть карточки заказа."),
	order: z
		.object({
			menuItems: z
				.array(
			...
			hasAllergy: z
				...
				// .optional()
				.describe(
					"Есть ли у меня аллергия на какие либо продукты."
				),
			allergy: z
				.string()
				.array()
			...
			phoneNumber: z
				...
				.or(z.null())
				.describe(
					"Номер телефона, по которому со мной можно будет связаться."
				),
			paymentMethod: z
				.enum(["Карта", "Наличные"])
				.or(z.undefined())
...
Твоя задача: собрать информацию о моем заказе. Сначала узнай блюда, которые я хочу заказать. Спрашивай хочу ли я дополнить заказ до тех пор, пока я не откажусь. После этого узнай всю остальную информацию о заказе: наличие аллергии, время забора, номер телефона, способ оплаты, на какое количество персон положить столовые приборы. Не формируй карточку заказа до тех пор, пока я не скажу тебе все эти данные.
В твоих ответах должна содержаться информация о еде, которую я заказываю, но не называй мне их цену. Если ты не найдешь такой еды с помощью своих инструментов, скажи что такого блюда в меню нет. Твой ответ никогда не должен содержать <br>. В твоем ответе не должно содержаться больше одного вопроса. Никогда не используй символ <br>.
...
Если я спрашиваю о еде, найди всю информацию об этом с помощью своих инструментов.
...
После того, как ты выяснишь все детали моего заказа, уточни у меня все ли правильно. Только после того, как я соглашусь, отправь мне карточку заказа. Представь, что внутри твоего ответа два отдельных сообщения - в одном ты говоришь, что заказ принят, а во втором отправляешь мне карточку моего заказа.
